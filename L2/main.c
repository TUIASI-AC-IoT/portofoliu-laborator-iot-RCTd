...
#define CONFIG_PEER_IP_ADDR "192.168.89.50"
...
static void udp_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            char *response = "GPIO4=1";
            response[6]=x+'0';
            int len = sendto(sock, response, strlen(response), 0,(struct sockaddr *)&dest_addr, sizeof(dest_addr));
            ...
            else
            {
                ...
                // ESP_LOGI(TAG, "Send %d bytes to %s:", len, addr_str);
                // ESP_LOGI(TAG, "%s", rx_buffer);

                // gpio_set_level(GPIO_OUTPUT_IO, (rx_buffer[len-1]-'0'));
